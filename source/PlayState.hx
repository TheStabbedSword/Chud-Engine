...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'senpai':
				dialogue = CoolUtil.coolTextFile('assets/data/charts/senpai/senpaiDialogue.txt');
			case 'roses':
				dialogue = CoolUtil.coolTextFile('assets/data/charts/roses/rosesDialogue.txt');
			case 'thorns':
				dialogue = CoolUtil.coolTextFile('assets/data/charts/thorns/thornsDialogue.txt');
		}
		...
		scoreTxt = new FlxText(0, healthBarBG.y + 35, 0, "", 20);
		scoreTxt.setFormat("assets/fonts/vcr.ttf", 16, FlxColor.WHITE, CENTER, OUTLINE, 0xFF000000);
		...
		for (i in [strumLineNotes, notes, healthBar, healthBarBG, iconP1, iconP2, scoreTxt, doof])
		{
			i.camera = camHUD;
		}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices) {
			var vocalPath = Paths.voices(SONG.song);
			trace(vocalPath);

			vocals = new FlxSound();
			vocals.loadEmbedded(vocalPath);
		} else {
			vocals = new FlxSound();
		}
	...
	override public function update(elapsed:Float)
	{
		...
		scoreTxt.text = TextManager.getTextString('scoreTxt') + ": " + songScore + " | " + TextManager.getTextString('missesTxt') + ": " + misses + " | " + TextManager.getTextString('accuracyTxt') + ": " + truncateFloat(accuracy, 2) + "% | "  + TextManager.getTextString('comboTxt') +  ": " + combo;
		scoreTxt.screenCenter(X);
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					cpuStrums.forEach(function(sprite:FlxSprite)
					{
