...
class OptionsState extends MusicBeatState
{
	...
	private var grpChoices:FlxTypedGroup<Alphabet>;
	...
	override function create()
	{
		...
		grpChoices = new FlxTypedGroup<Alphabet>();
		add(grpChoices);

		for (i in 0...optionList.length)
		{
			var controlLabel:Alphabet = new Alphabet(0, 0, optionList[i], true, false);
			controlLabel.screenCenter();
			controlLabel.y += (70 * i) + 30;
			controlLabel.ID = i;
			grpChoices.add(controlLabel);
		}
		changeSelection();
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.ACCEPT) {
			for (item in grpChoices.members) {
				item.alpha = 0;
			}

			switch(optionList[curSelected].toLowerCase()) {
				case 'Controls':
	...
	function changeSelection(change:Int = 0)
	{
		...
		if (curSelected < 0)
			curSelected = grpChoices.length - 1;
		if (curSelected >= grpChoices.length)
			curSelected = 0;
		...
		grpChoices.forEach(function(spr:Alphabet) {
	...
	{
		var selected:String = optionList[curSelected];
...
class ControlsSubstate extends MusicBeatSubstate {
	...
	var optionShit:Array<String> = [
		'NOTES',
		ClientOptions.keyBinds[0][1],
		ClientOptions.keyBinds[1][1],
		ClientOptions.keyBinds[2][1],
		ClientOptions.keyBinds[3][1],
		'',
		'UI',
		ClientOptions.keyBinds[4][1],
		ClientOptions.keyBinds[5][1],
		ClientOptions.keyBinds[6][1],
		ClientOptions.keyBinds[7][1],
		'',
		defaultKey];
	...
	public function new() {
		...
		controlArray = ClientOptions.lastControls.copy();
		for (i in 0...optionShit.length) {
			...
			@:privateAccess optionText.yMulti = 60;
	...
	override function update(elapsed:Float) {
		if(rebindingKey < 0) {
			...
			if (controls.BACK) {
				ClientOptions.reloadControls(controlArray);
			...
			if(controls.ACCEPT && nextAccept <= 0) {
				if(optionShit[curSelected] == defaultKey) {
					controlArray = ClientOptions.defaultKeys.copy();
					reloadKeys();
					changeSelection();
	...
	private function unselectableCheck(num:Int, ?checkDefaultKey:Bool = false):Bool {
		...
		for (i in 0...ClientOptions.keyBinds.length) {
			if(ClientOptions.keyBinds[i][1] == optionShit[num]) {
				return false;
			}
		}
		return true;
	...
	private function getSelectedKey():Int {
		...
		for (i in 0...ClientOptions.keyBinds.length) {
			if(ClientOptions.keyBinds[i][1] == optionShit[curSelected]) {
				return i*2 + altValue;
			}
		}
		return -1;
